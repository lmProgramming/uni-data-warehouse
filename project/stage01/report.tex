...
\begin{itemize}[label=\textbullet, leftmargin=*]
  ...
  \item \textbf{Kierowcy:} Dane demograficzne (narodowość, data urodzenia \(\rightarrow\) wiek w momencie wyścigu), historia startów, przynależność zespołowa w poszczególnych wyścigach.
...
\begin{itemize}[label=\textbullet, leftmargin=*]
  ...
  \item \textbf{Drugie źródło (Minute by Minute F1 Grand Prix Weather Data):} Ten zbiór dostarcza szczegółowych, minutowych danych pogodowych dla Grand Prix Formuły 1 w latach 2018-2023. Zawiera takie informacje jak temperatura powietrza i toru, wilgotność, ciśnienie, opady, kierunek i prędkość wiatru.
        \begin{itemize}[label=\textendash, leftmargin=1em]
          ...
          \item \textbf{Mapowanie torów:} Chociaż dane pogodowe zawierają \texttt{Year} i \texttt{Round Number}, co powinno wystarczyć do jednoznacznej identyfikacji wyścigu, warto będzie zweryfikować spójność identyfikacji torów. Plik \texttt{races.csv} zawiera \texttt{circuitId} (numeryczny), który łączy się z \texttt{circuits.csv} (zawierającym m.in. \texttt{circuitRef} - tekstowy identyfikator toru). Jeśli dane pogodowe zawierałyby tekstowy identyfikator toru, konieczne byłoby jego mapowanie na \texttt{circuitRef} lub \texttt{circuitId} dla zapewnienia pełnej spójności i możliwości wzbogacania analiz.
        \end{itemize}
        Ten zbiór danych znacząco wzbogaci możliwości analityczne dla nowszych sezonów, pozwalając na badanie korelacji między warunkami atmosferycznymi a wynikami czy strategiami.
\end{itemize}
...
% --- Plik: constructor_standings.csv ---
...
\begin{enumerate}[label=\arabic*., wide, labelwidth=!, labelindent=0pt]
  \item \textbf{\texttt{Dim\_Driver}}
        \begin{itemize}[label=\textendash, leftmargin=1em]
          ...
          \item Atrybuty: \texttt{Forename}, \texttt{Surname}, \texttt{FullName}, \texttt{DateOfBirth}, \texttt{AgeAtRace} (wyliczane), \texttt{Nationality}
          \item Hierarchie: Potencjalnie: \texttt{Nationality \(\rightarrow\) Continent} (jeśli dodamy mapowanie kontynentów).
        \end{itemize}

  \item \textbf{\texttt{Dim\_Constructor}}
        \begin{itemize}[label=\textendash, leftmargin=1em]
          \item Klucze: \texttt{ConstructorKey} (PK), \texttt{ConstructorID\_NK}, \texttt{ConstructorRef}
          \item Atrybuty: \texttt{Name}, \texttt{Nationality}
        \end{itemize}

  \item \textbf{\texttt{Dim\_Race}} (wydarzenie wyścigowe)
        \begin{itemize}[label=\textendash, leftmargin=1em]
          \item Klucze: \texttt{RaceKey} (PK), \texttt{RaceID\_NK}
          \item Atrybuty: \texttt{YearSeason} (Sezon), \texttt{RoundNumberInSeason}, \texttt{RaceNameOfficial} (np. "Monaco Grand Prix")
          \item Klucze obce: \texttt{CircuitKey} (FK do Dim\_Circuit), \texttt{DateKey} (FK do Dim\_Time - data wyścigu)
  ...
  \item \textbf{\texttt{Dim\_Time}} (dla daty wyścigu)
        \begin{itemize}[label=\textendash, leftmargin=1em]
          \item Klucze: \texttt{DateKey} (PK, np. YYYYMMDD)
          \item Atrybuty: \texttt{FullDateAlternateKey} (DATE), \texttt{Year}, \texttt{Quarter}, \texttt{Month}, \texttt{MonthName}, \texttt{DayOfMonth}, \texttt{DayOfWeekName}, \texttt{WeekOfYear}
          \item Hierarchie: \texttt{Year \(\rightarrow\) Quarter \(\rightarrow\) Month \(\rightarrow\) DayOfMonth}; \texttt{Year \(\rightarrow\) WeekOfYear}.
        \end{itemize}

  \item \textbf{\texttt{Dim\_Weather\_Aggregated}} (zagregowane dane pogodowe dla wyścigu, dla lat 2018-2023)
        \begin{itemize}[label=\textendash, leftmargin=1em]
          \item Klucze: \texttt{WeatherKey} (PK)
          \item Klucze naturalne powiązania: \texttt{Year\_NK}, \texttt{RoundNumber\_NK} (do łączenia z \texttt{Dim\_Race} przed załadowaniem faktów)
          \item Atrybuty/Miary zagregowane: \texttt{AvgAirTempCelsius}, \texttt{MinAirTempCelsius}, \texttt{MaxAirTempCelsius}, \texttt{AvgTrackTempCelsius}, \texttt{AvgHumidityPercent}, \texttt{DidRainOccur} (Boolean), \texttt{AvgWindSpeedKmph}, \texttt{MaxWindSpeedKmph}, \texttt{DominantWindDirectionCategory}
        \end{itemize}

  \item \textbf{\texttt{Dim\_Status}} (opisuje sposób zakończenia wyścigu przez kierowcę)
        \begin{itemize}[label=\textendash, leftmargin=1em]
          \item Klucze: \texttt{StatusKey} (PK), \texttt{StatusID\_NK}
          \item Atrybuty: \texttt{StatusDescription} (np. "Finished", "+1 Lap", "Engine", "Collision")
          \item Hierarchie: Możliwe grupowanie statusów, np. \texttt{StatusCategory (\text{Ukończył}, \text{Awaria}, \text{Wypadek}) \(\rightarrow\) StatusDescription}.
        \end{itemize}
\end{enumerate}
...
\begin{figure}[H]
  \centering
  % \includegraphics[width=0.9\textwidth]{images/schemat_gwiazdy.png} % Odkomentuj i wstaw swój diagram
  \fbox{\parbox[c][10cm][c]{0.9\textwidth}{\centering Miejsce na diagram schematu gwiazdy \\ (np. obrazek .png lub kod TikZ)}}
...
\begin{itemize}[label=\textbullet, leftmargin=*]
  \item \textbf{Addytywne:} \texttt{PointsScored}, \texttt{LapsCompleted}, \texttt{NumberOfPitStops}.
  \item \textbf{Póładdytywne:} \texttt{RaceTimeMilliseconds} (addytywna w kontekście sumowania czasów segmentów dla tego samego kierowcy/wyścigu, ale nie między różnymi wyścigami/kierowcami jako prosta suma).
  \item \textbf{Nieaddytywne:} \texttt{GridPosition}, \texttt{FinalPositionOrder}, \texttt{FastestLapTimeMilliseconds} (agregowane przez MIN), \texttt{RankFastestLap}. Wszelkie miary procentowe lub współczynniki wyliczane na podstawie innych miar również będą nieaddytywne lub póładdytywne.
\end{itemize}
