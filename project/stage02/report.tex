...
\begin{footnotesize}
  ...
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    RaceKey              & results.xlsx, Dim\_Race        & RaceKey                     & Lookup z Dim\_Race.
    \\
    \hline
    DriverKey            & results.xlsx, Dim\_Driver      & DriverKey                   & Lookup z Dim\_Driver (res.driverId) -> DriverKey (SK). FK.
    \\
    \hline
    ConstructorKey       & results.xlsx, Dim\_Constructor & ConstructorKey              & Lookup z Dim\_Constr. (res.constructorId) -> ConstrKey (SK). FK.
    \\
    \hline
    CircuitKey           & results.xlsx, Dim\_Circuit     & CircuitKey                  & Lookup z Dim\_Circuit
    \\
    \hline
    DateKey              & results.xlsx, Dim\_Race        & Date                        & Lookup z Dim\_Race -> FullDate. Derived Column: (YEAR(FullDate)*10000) + (MONTH(FullDate)*100) + DAY(FullDate). INT YYYYMMDD. \\
    \hline
    WeatherKey           & {Dim\_Weather} / Dim\_Race     & races.raceId LUB races.year & Lookup z Dim\_Race -> RaceID\_NK (lub Year,Round). LkUp Dim\_Weather. Der.Col: NULL jeśli rok < 2018. FK.
    \\
    \hline
    StatusKey            & Dim\_Status                    & results.statusId (NK)       & Lookup z Dim\_Status (res.statusId) -> StatusKey (SK). FK.
    \\
    \hline
    ...
    \\
    \hline
    NumberOfPitStops     & pit\_stops.xlsx                & raceId, driverId            & Pod-przepływ: Agg. (COUNT(stop) jako stopsCount). Merge Join (Left Outer). Der.Col: ISNULL(stopsCount) ? 0 : stopsCount.      \\
    \hline
    RaceTimeMilliseconds & results.xlsx                   & milliseconds LUB time       & Mapa bezp. LUB Der.Col (parsuj tekst time) + DC (do DT\_I8). NULL jeśli DNF.
    \\
    \hline
    GridPosition         & results.xlsx                   & grid                        & Mapa bezp. + DC (do DT\_I4). Obsługa 0 dla pit lane.
    \\
    \hline
    FinalPositionOrder   & results.xlsx                   & positionOrder LUB position  & Mapa bezp. + DC (do DT\_I4).
    \\
    \hline
    PositionOrderChange  & results.xlsx                   & grid, positionOrder         & Der.Col: GridPosition - FinalPositionOrder.
    \\
    \hline
    RankFastestLap       & results.xlsx                   & rank (fast lap)             & Mapa bezp. + DC (do DT\_I4). NULL jeśli brak.
    \\
    \hline
    FastestLapTopSpeed   & results.xlsx                   & fastestLapSpeed             & Mapa bezp. + DC (do DT\_DECIMAL). NULL jeśli brak.
    \\
    \hline
    AgeAtRace            & Dim\_Driver, Dim\_Race         & drivers.dob, races.date     & LkUp Dim\_Driver (dob). LkUp Dim\_Race (date). Der.Col: Kalkulacja wieku. NULL jeśli dob NULL.                                \\
  \end{longtable}
\end{footnotesize}
