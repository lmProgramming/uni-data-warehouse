...
\begin{footnotesize}
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    \hline
    \texttt{CircuitID\_NK} & \texttt{circuits.xlsx} & \texttt{circuitId}      & Mapa bezpośrednia + DC (do INT). Klucz naturalny.   \\
    \hline
    \texttt{CircuitName}   & \texttt{circuits.xlsx} & \texttt{name}           & Mapa bezpośrednia + DC (do \texttt{DT\_WSTR}).      \\
    \hline
    \texttt{LocationCity}  & \texttt{circuits.xlsx} & \texttt{location}       & Mapa bezpośrednia + DC (do \texttt{DT\_WSTR}).      \\
    \hline
    \texttt{CountryName}   & \texttt{circuits.xlsx} & \texttt{country}        & Mapa bezpośrednia + DC (do \texttt{DT\_WSTR}).      \\
  \end{longtable}
\end{footnotesize}
...
\begin{footnotesize}
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    \toprule
    \textbf{Atrybut}       & \textbf{Źródło / Metoda}                           & \textbf{Kolumna(y) Źródła / Logika} & \textbf{Transformacje i uwagi}                                                                                                          \\
    \midrule
    \endfirsthead
    \multicolumn{4}{c}%
    {{\bfseries \tablename\ \thetable{} -- Kontynuacja z poprzedniej strony}}                                                                                                                                                                                   \\
    ...
    \bottomrule
    \endlastfoot
    \texttt{DateKey}       & \texttt{races.xlsx} (distinct dates) LUB Generacja & \texttt{date} LUB iteracja          & Der. Col.: \texttt{(YEAR(FullDate) * 10000) + (MONTH(FullDate) * 100) + DAY(FullDate)}. PK, YYYYMMDD INT.                               \\
    \hline
    \texttt{FullDate}      & \texttt{races.xlsx} (distinct dates) LUB Generacja & \texttt{date} LUB iteracja          & Mapa bezp. LUB generacja + DC (do \texttt{DT\_DATE}).                                                                                   \\
    \hline
    \texttt{Year}          & j.w.                                               & Bazuje na \texttt{FullDate}         & Der. Col.: \texttt{YEAR(FullDate)}.                                                                                                     \\
    \hline
    \texttt{Quarter}       & j.w.                                               & Bazuje na \texttt{FullDate}         & Der. Col.: \texttt{DATEPART("qq", FullDate)}.                                                                                           \\
    \hline
    \texttt{Month}         & j.w.                                               & Bazuje na \texttt{FullDate}         & Der. Col.: \texttt{MONTH(FullDate)}.                                                                                                    \\
    \hline
    \texttt{MonthName}     & j.w. \newline \texttt{Helper\_Months}              & Bazuje na numerze miesiąca          & Der. Col.: \texttt{MONTH(FullDate)}; \newline LkUp na \texttt{Helper\_Months}. Polska nazwa.                                            \\
    \hline
    \texttt{DayOfMonth}    & j.w.                                               & Bazuje na \texttt{FullDate}         & Der. Col.: \texttt{DAY(FullDate)}.                                                                                                      \\
    \hline
    \texttt{DayOfWeekName} & j.w. \newline \texttt{Helper\_Weekdays}            & Bazuje na \texttt{FullDate}         & Der. Col.: \texttt{DATEPART("dw", FullDate)}; \newline LkUp na \texttt{Helper\_Weekdays}. Polska nazwa. Weryfikacja \texttt{DATEFIRST}. \\
  \end{longtable}
\end{footnotesize}
...
\begin{footnotesize}
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    \bottomrule
    \endlastfoot
    \texttt{WeatherKey}             & -                                & -                                              & SQL Server \texttt{IDENTITY(1,1)}. Klucz zastępczy.                                               \\
    \hline
    \texttt{DidRainOccur}           & \texttt{F1 Weather (\dots).xlsx} & \texttt{Rainfall} (np. \texttt{MAX} flagi 0/1) & Agg. \newline Der. Col. (\texttt{AggRain > 0 ? "Tak" : "Nie"}). "Tak"/"Nie".                      \\
    \hline
    \texttt{WindSpeedCategory}      & \texttt{F1 Weather (\dots).xlsx} & \texttt{WindSpeed} (np. \texttt{AVG})          & Agg. \newline Der. Col. (Logika CASE dla kategorii: "Słaby", "Umiarkowany", "Silny").             \\
    \hline
    \texttt{AirTempCategory}        & \texttt{F1 Weather (\dots).xlsx} & \texttt{AirTemp} (np. \texttt{AVG})            & Agg. \newline Der. Col. (Logika CASE dla kategorii: "Zimno", "Umiarkowanie", "Ciepło", "Gorąco"). \\
    \hline
    \texttt{TrackTempCategory}      & \texttt{F1 Weather (\dots).xlsx} & \texttt{TrackTemp} (np. \texttt{AVG})          & Agg. \newline Der. Col. (Logika CASE dla kategorii).                                              \\
    \hline
    \texttt{HumidityCategory}       & \texttt{F1 Weather (\dots).xlsx} & \texttt{Humidity} (np. \texttt{AVG})           & Agg. \newline Der. Col. (Logika CASE dla kategorii: "Niska", "Średnia", "Wysoka").                \\
    \hline
    \texttt{PressureCategory}       & \texttt{F1 Weather (\dots).xlsx} & \texttt{Pressure} (np. \texttt{AVG})           & Agg. \newline Der. Col. (Logika CASE dla kategorii: "Niskie", "Normalne", "Wysokie").             \\
    \hline
    *(Implied)* \texttt{RaceID\_NK} & \texttt{F1 Weather (\dots).xlsx} & \texttt{Year}, \texttt{Round Number}           & Agg. (Group by). Służy do połączenia z wyścigiem w Fact. Dla 2018-2023.                           \\
  \end{longtable}
\end{footnotesize}
...
\begin{footnotesize}
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    \hline
    \texttt{StatusID\_NK}      & \texttt{status.xlsx}                                  & \texttt{statusId}                        & Mapa bezpośrednia + DC (do INT). Klucz naturalny.                                                                     \\
    \hline
    \texttt{StatusDescription} & \texttt{status.xlsx}                                  & \texttt{status}                          & Mapa bezpośrednia + DC (do \texttt{DT\_WSTR}).                                                                        \\
    \hline
    \texttt{StatusCategory}    & \texttt{status.xlsx} \newline \texttt{Helper\_Status} & \texttt{status} (z \texttt{status.xlsx}) & LkUp na \texttt{Helper\_Status} (\texttt{status.status} -> \texttt{BroadCategory}). Kategoria np. Finished, Accident. \\
  \end{longtable}
\end{footnotesize}
...
\begin{footnotesize}
  ...
  \begin{longtable}{|L{3.5cm}|L{3.5cm}|L{3cm}|L{7cm}|}
    ...
    \toprule
    \textbf{Atrybut}              & \textbf{Źródło CSV / Wymiar}               & \textbf{Kolumna(y) Źródła / Logika}                       & \textbf{Transformacje i uwagi}                                                                                                                               \\
    \midrule
    \endfirsthead
    \multicolumn{4}{c}%
    {{\bfseries \tablename\ \thetable{} -- Kontynuacja z poprzedniej strony}}                                                                                                                                                                                                                             \\
    ...
    \texttt{RaceKey}              & \texttt{Dim\_Race}                         & \texttt{results.raceId} (NK)                              & LkUp \texttt{Dim\_Race} (\texttt{res.raceId}) -> \texttt{RaceKey} (SK). FK. HWM: \texttt{results.xlsx} filtrowane po \texttt{resultId}.                      \\
    \hline
    \texttt{DriverKey}            & \texttt{Dim\_Driver}                       & \texttt{results.driverId} (NK)                            & LkUp \texttt{Dim\_Driver} (\texttt{res.driverId}) -> \texttt{DriverKey} (SK). FK.                                                                            \\
    \hline
    \texttt{ConstructorKey}       & \texttt{Dim\_Constructor}                  & \texttt{results.constructorId} (NK)                       & LkUp \texttt{Dim\_Constr.} (\texttt{res.constructorId}) -> \texttt{ConstrKey} (SK). FK.                                                                      \\
    \hline
    \texttt{CircuitKey}           & \texttt{Dim\_Circuit}                      & \texttt{races.circuitId} (NK, z LkUp \texttt{Dim\_Race})  & LkUp \texttt{Dim\_Race} -> \texttt{CircuitID\_NK}; LkUp \texttt{Dim\_Circuit} (\texttt{CircuitID\_NK}) -> \texttt{CircuitKey} (SK). FK.                      \\
    \hline
    \texttt{DateKey}              & \texttt{Dim\_Race} / Kalk.                 & \texttt{races.date} (FullDate, z LkUp \texttt{Dim\_Race}) & LkUp \texttt{Dim\_Race} -> \texttt{FullDate}. Der.Col: \texttt{(YEAR(FullDate)*10000) + (MONTH(FullDate)*100) + DAY(FullDate)}. INT YYYYMMDD.                \\
    \hline
    \texttt{WeatherKey}           & \texttt{Dim\_Weather} / \texttt{Dim\_Race} & \texttt{races.raceId} LUB \texttt{races.year}             & LkUp \texttt{Dim\_Race} -> \texttt{RaceID\_NK} (lub \texttt{Year,Round}). LkUp \texttt{Dim\_Weather}. Der.Col: NULL jeśli rok < 2018. FK.                    \\
    \hline
    \texttt{StatusKey}            & \texttt{Dim\_Status}                       & \texttt{results.statusId} (NK)                            & LkUp \texttt{Dim\_Status} (\texttt{res.statusId}) -> \texttt{StatusKey} (SK). FK.                                                                            \\
    \hline
    \texttt{PointsScored}         & \texttt{results.xlsx}                      & \texttt{points}                                           & Mapa bezp. + DC (do \texttt{DT\_DECIMAL}).                                                                                                                   \\
    \hline
    \texttt{LapsCompleted}        & \texttt{results.xlsx}                      & \texttt{laps}                                             & Mapa bezp. + DC (do \texttt{DT\_I4}).                                                                                                                        \\
    \hline
    \texttt{NumberOfPitStops}     & \texttt{pit\_stops.xlsx} (Agg.)            & \texttt{raceId}, \texttt{driverId}                        & Pod-przepływ: Agg. (\texttt{COUNT(stop)} jako \texttt{stopsCount}). \texttt{Merge Join} (Left Outer). Der.Col: \texttt{ISNULL(stopsCount) ? 0 : stopsCount}. \\
    \hline
